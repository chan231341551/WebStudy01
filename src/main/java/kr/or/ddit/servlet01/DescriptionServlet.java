...
/**
 * 서블릿 : 웹상에서 발생하는 요청을 처리하고 , 그에 따른 동적 응답을 생성하기 위한 객체의 필요조건(명세, specification)
 * 개발단계 
 * 	1. HttpServlet 상속
 * 	2. callback 메소드 재정의
 ...
 *	6. 컨테이너 재구동
 *
 *	** servlet container? (객체의 생명주기 관리)
 *		: 컨테이너에 의해 관리되는 서블릿 객체의 생명주기 제어.
 *
 *	서블릿 객체의 싱글턴 인스턴스 생성 : 구체적인 설정(loadOnStartup=1)이 없는 한, 매핑된 조건의 요청이 발생했을떄 생성. (1 = 서블릿 생성 순서)
 *	
 ...
 *
 *	callback : 관련 이벤트가 발생했을떄 시스템 내부적으로 자동 호출되는 코드 형태 .
...
public class DescriptionServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		super.service(req, resp); //doGet 호출
